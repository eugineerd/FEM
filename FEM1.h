...
const double h = 10.0e10;
const double E = 10.0e11;
const double f = 10.0e11;
...
public:
  ...
  // Solution steps
  void generate_mesh(unsigned int numberOfElements, double L_ = 0.1);
  void define_boundary_conds();
  void setup_system(double g1_ = 0, double g2_ = 0.001,
                    unsigned int quadRule_ = 2);
...
double FEM<dim>::basis_gradient(unsigned int node, double xi) {
  ...
  double value = 0.; // Store the value of the gradient of the basis function in
                     // this variable

  /*You can use the function "xi_at_node" (defined above) to get the value of xi
    (in the bi-unit domain) at any node in the element - using deal.II's element
    node numbering pattern.*/
  double xi0, xi1, xi2, xi3, denum;
  ...
  case 3:
    ...
    denum = (xi0 - xi1) * (xi0 - xi2) * (xi0 - xi3);
    value = (xi - xi2) * (xi - xi3) / denum + (xi - xi1) * (xi - xi3) / denum +
            (xi - xi1) * (xi - xi2) / denum;
    break;
  }
...
template <int dim>
void FEM<dim>::generate_mesh(unsigned int numberOfElements, double L_) {
...
template <int dim>
void FEM<dim>::setup_system(double g1_, double g2_, unsigned int quadRule_) {
...
// matrix (K)
template <int dim> void FEM<dim>::assemble_system() {
  ...
  for (; elem != endc; ++elem) {
    ...
    /*We find the element length by subtracting the x-coordinates of the two end
      nodes of the element. Remember that the vector "nodeLocation" holds the
      x-coordinates, indexed by the global node number. "local_dof_indices"
      gives us the global node number indexed by the element node number.*/
    h_e =
    ...
    for (unsigned int A = 0; A < dofs_per_elem; A++) {
      for (unsigned int q = 0; q < quadRule; q++) {
        x = 0;
        // Interpolate the x-coordinates at the nodes to find the x-coordinate
        // at the quad pt.
  ...
  /*deal.II applies Dirichlet boundary conditions (using the boundary_values map
    we defined in the function "define_boundary_conds") without resizing K or
    F*/
...
template <int dim> double FEM<dim>::l2norm_of_error() {
  ...
  for (; elem != endc; ++elem) {
    ...
    for (unsigned int q = 0; q < quadRule; q++) {
      ...
      if (prob == 1)
        u_exact = -x * x * x * f / (6. * E) +
                  (g2 - g1 + L * L * L * f / (6. * E)) / L * x + g1;

      if (prob == 2)
        u_exact =
            -x * x * x * f / (6. * E) + (h + 0.5 * L * L * f) / E * x + g1;
